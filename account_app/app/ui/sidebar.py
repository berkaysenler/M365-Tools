...
class SidebarFrame(tk.Frame):
    def __init__(self, parent, rto_list, on_select, on_connect):
        super().__init__(parent, width=190, bg=C_PANEL)
    ...
    def _build(self, rto_list):
        ...
        tk.Label(
            self, text="Tenants", bg=C_PANEL, fg=C_TEXT,
            font=("Segoe UI", 11, "bold"), anchor="w", padx=10, pady=10,
        ).grid(row=0, column=0, sticky="ew")

        tk.Frame(self, bg=C_BORDER, height=1).grid(row=1, column=0, sticky="ew")

        tk.Label(
            self, text="RTO TENANTS", bg=C_PANEL, fg=C_DIM,
        ...
        for i, rto in enumerate(rto_list):
            btn = tk.Button(
                ...
                bg=C_PANEL, fg=C_TEXT, activebackground=C_PANEL_HOVER,
        ...
        tk.Frame(self, bg=C_BORDER, height=1).grid(row=sep_row, column=0, sticky="ew", pady=(8, 0))

        self._connect_btn = tk.Button(
            self, text="Connect", bg=C_SELECTED, fg=C_TEXT,
            activebackground=C_ACCENT_HOV, activeforeground=C_TEXT,
    ...
    def _select(self, rto):
        if self._selected and self._selected in self._buttons:
            self._buttons[self._selected].config(bg=C_PANEL, fg=C_TEXT)
        self._selected = rto
        self._buttons[rto].config(bg=C_PANEL_HOVER, fg=C_TEXT)
    ...
    def set_connect_state(self, connected):
        if connected:
            ...
            if self._selected:
                self._buttons[self._selected].config(bg="#1e4d2b", fg=C_OK)
        else:
            self._connect_btn.config(text="Connect", bg=C_SELECTED, activebackground=C_ACCENT_HOV)
            if self._selected:
                self._buttons[self._selected].config(bg=C_PANEL_HOVER, fg=C_TEXT)
