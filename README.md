...
2. Click `Add account` for each tenant you manage (ORG name, admin email, domain).
...
If a single Microsoft 365 tenant has multiple verified domains, add each domain as its own ORG entry, then add a `"manages"` list to the primary entry in `config.json` (e.g. `"manages": ["ORG2", "ORG3"]`). Those entries then share the primary account's sign-in instead of needing their own.
